...
def generate_recommendations(user, target_allocation=None, annual_goal=None, projections=None):
        ...
        if pct > 40:
            ...
            recs.append(
                InvestmentRecommendation(
                    user=user, investment=None, recommendation_type='rebalance', message=message
                )
            )
    ...
    total_debt = (
        Debt.objects.filter(account__user=user).aggregate(Sum('balance'))['balance__sum'] or 0
    )
    if not savings_accounts.exists() and total_debt > 0:
        ...
        recs.append(
            InvestmentRecommendation(
                user=user, investment=None, recommendation_type='emergency_fund', message=message
            )
        )
    ...
    if not has_tax_adv:
        ...
        recs.append(
            InvestmentRecommendation(
                user=user, investment=None, recommendation_type='tax_advantaged', message=message
            )
        )
    return recs
